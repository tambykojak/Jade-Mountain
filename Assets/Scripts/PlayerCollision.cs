...
public class PlayerCollision : MonoBehaviour
{
    ...
    private bool IsAngleSafeForLanding()
    {
        ...
        if (!safe)
        {
            Debug.Log("Angle wasn't safe. Your angle was " + angle + ". A safe angle is greater than " + (360 - safeDegrees) + " (-" + safeDegrees + ") or less than " + safeDegrees + ".");
        }
        return safe;
    }
    ...
    private bool IsSpeedSafeForLanding()
    {
        ...
        if (!safe)
        {
            Debug.Log("Velocity wasn't safe. Your velocity when colliding was " + lastVelocity + ". The max safe velocity is " + safeMaxVelocity);
        }
        return safe;
    }
}
